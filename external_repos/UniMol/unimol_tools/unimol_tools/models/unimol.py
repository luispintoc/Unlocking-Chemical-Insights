...
class UniMolModel(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        ###########################################
        encoder_rep, attn_mask, delta_pair_repr, x_norm, delta_pair_repr_norm, intermediate_reprs = (
            self.encoder(x, padding_mask=padding_mask, attn_mask=graph_attn_bias, layer_idx=layer_idx)
        )
        ###########################################
