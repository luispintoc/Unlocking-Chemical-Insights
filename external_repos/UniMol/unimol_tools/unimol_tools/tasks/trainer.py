    ...
    def inference_with_ddp(
        ...
        for batch in tqdm(dataloader):
            ...
            with torch.no_grad():
                ...
                repr_dict["intermediate_layer_reprs"].extend(
                    item.cpu().numpy() for item in outputs["intermediate_layer_reprs"]
                )
        ...
        if local_rank == 0:
            ...
            if return_atomic_reprs:
                merged_repr_dict.update(
                    {"atomic_symbol": [], "atomic_coords": [], "atomic_reprs": [], "intermediate_layer_reprs": []}
                )

            for rd in gathered_list:
                merged_repr_dict["cls_repr"].extend(rd["cls_repr"])
                merged_repr_dict["intermediate_layer_reprs"].extend(rd.get("intermediate_layer_reprs", []))
                if return_atomic_reprs:
            ...
            merged_repr_dict["intermediate_layer_reprs"] = merged_repr_dict["intermediate_layer_reprs"][: len(dataset)]
            if return_atomic_reprs:
